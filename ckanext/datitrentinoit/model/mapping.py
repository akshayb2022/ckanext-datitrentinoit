...
from ckanext.datitrentinoit.model.statweb_metadata import (
    StatWebMetadataPro,
    StatWebMetadataSubPro,
    StatWebProEntry,
)
...
cat_map_pro = {
    ...
    "commercio con l'estero": "economia",
    ...
    "ricerca": "cultura",
    "sviluppo e innovazione": "scienza-tecnologia",
    "mercato del lavoro": "popolazionesocieta",
    "salute": "wellbeing",
    "famiglie e comportamenti sociali": "wellbeing",
    "assistenza e protezione sociale": "wellbeing",
    "popolazione": "popolazionesocieta",
    "società dell'informazione": "popolazionesocieta",
}
...
cat_map_sub = {
    ...
    "le imprese, la formazione e la valorizzazione del capitale produttivo": "economia",
...
def create_base_dict(guid, metadata, config):
    ...
    license = License.q().filter(
        License.default_name.like(lic_search)
    ).first() or License.get(License.DEFAULT_LICENSE)
    ...
    extras = {
        ...
        "themes_aggregate": [
            {"subthemes": [], "theme": metadata.get_tema() or "OP_DATPRO"}
        ],
...
def create_pro_package_dict(
    guid, swpentry: StatWebProEntry, metadata: StatWebMetadataPro, config
) -> dict:
    """
    ...
    groupname = cat_map_pro.get(
        (metadata.get_settore() or "default").lower(), DEFAULT_GROUP_PRO
    )
...
def create_subpro_package_dict(guid, metadata, config):
    ...
    groupname = cat_map_sub.get(
        (metadata.get_settore() or "default").lower(), DEFAULT_GROUP_SUBPRO
    )
    groups = [{"name": groupname}]
...
def create_subpro_description(metadata):
    ...
    d = _add_field(d, "Algoritmo", metadata.get_algoritmo())
    d = _add_field(
        d, "Tipo Indicatore", tipoindicatore_map.get(metadata.get_tipo_indicatore())
    )
